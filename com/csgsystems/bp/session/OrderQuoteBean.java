 ...
 * Generated  File: OrderQuoteBean.java
...
import com.csgsystems.bp.interfaces.OrderQuoteInterface;

/**
 * Comverse Product: BP.
 * Note: Some defaulted inputs depend on _REF tables having is_default rows and will not always be set.
 *       Conditional defaults/derivations/null-allowables are not listed here.
 * @ejb:bean name="OrderQuoteSessionBean"
 * display-name="OrderQuote Stateless Session EJB/>"
 * type="Stateless"
 * jndi-name="ejb/OrderQuoteBean"
...
@Stateless(name="OrderQuoteBean", mappedName = "OrderQuote")
@TransactionManagement(TransactionManagementType.BEAN)

@Path("OrderQuotes")

public class OrderQuoteBean implements OrderQuoteInterface
{
  protected ConnectionFactory cf       = null;
  protected SessionContext    sc       = null;
  @Inject protected BSDMSettings settings = null; 

  private BSDMInteractionSpec _OrderQuoteQuotePriceSpec = null;
  /**
  ...
  {
    return quotePrice (null, oqqpChargesIn, asOfDate, accountNo, invoiceCount);
  }
  /**
   * Retrieves bill invoices and specific charges for the order associated with the charge elemements passed in..
...
BSDMInteractionSpec   spec = new BSDMInteractionSpec (_OrderQuoteQuotePriceSpec, context);
...
record = cf.getRecordFactory ().createMappedRecord (null);
      if (oqqpChargesIn != null)
    {
      ...
      record.put ("OqqpChargesInList", list);
    }

      if (asOfDate != null) record.put ("AsOfDate", asOfDate);

      if (accountNo != null) record.put ("AccountNo", accountNo);

      if (invoiceCount != null) record.put ("InvoiceCount", invoiceCount);
...
return BillInvoiceObjectHelper.fromMapList (record, "OqqpBillInvoiceOutList");
}
  ...
  {
    return quotePrice (null, oqqpChargesIn, asOfDate, accountNo, invoiceCount, defaultAction, constraintAction);
  }
  /** 
   * Retrieves bill invoices and specific charges for the order associated with the charge elemements passed in..
   * @param context The session context to use when connecting to the APITS server.
  ...
  public BillInvoiceObjectDataList quotePrice (BSDMSessionContext context, ChargeElementObjectData[] oqqpChargesIn, Date asOfDate, Integer accountNo, Integer invoiceCount, String defaultAction, String constraintAction) throws BSDMResourceException, ConstraintException
  {
    ...
    //Constraints not supported on Input-Column
return quotePrice (context, oqqpChargesIn, asOfDate, accountNo, invoiceCount);
  }
  ...
  public OrderQuoteBean ()
  {
    try
    {
      _OrderQuoteQuotePriceSpec = new BSDMInteractionSpec("OrderQuoteQuotePrice", ApiMappings.getCallName ("OrderQuoteQuotePrice"), Boolean.FALSE);
   ...
   */
  @Inject public OrderQuoteBean(BSDMSettings settings)
  {
   ...
   * @param settings  BSDM connection settings to use when connecting to the TS/API middleware servers.
   */
  public OrderQuoteBean(BSDMSettings settings, BSDMConnectionManager cm)
  {
    this();
