"""  $Id: SOAP.py,v 1.1 2002/12/09 17:30:03 wcox Exp $
...
class SOAPUnmarshaler(Unmarshaler, saxlib.DocumentHandler):
    ...
    def decode(self):
        """ Decode first uses SOAPEnvelopeUM to unmarshal the SOAP
        envelope which in turn uses `self' to unmarshal the section 8
        stuff. """

        env_um = SOAPEnvelopeUM(self)
        self.parse_depth = 0
        self.um_init()
        self.parser.setDocumentHandler(env_um)
        self.parser.parseFile(self.stream)
        self.um_finish()
        o = self.parse_value_stack[0]
        delattr(self, 'parse_value_stack')
        return env_um.procedure, o
    ...
    def startElement(self, name, attrs):
        ...
        if (attrs.has_key('SOAP:arrayType')
            or (attrs.has_key('type')
                and attrs['type'] == 'list')
            or (attrs.has_key('csg:type')
                and attrs['csg:type'] == 'list')):
...
class SOAPEnvelopeUM(saxlib.DocumentHandler):
    def __init__(self, sub_marshaler):
        self.sub_marshaler = sub_marshaler
        sub_marshaler.parentDocumentHandler = self
        self.awaiting_method_tag = 0

    def startElement(self, name, attrs):
        if self.awaiting_method_tag:
            self.procedure = name
            self.sub_marshaler.parser.setDocumentHandler(self.sub_marshaler)
        elif name == "SOAP:Body":
            self.awaiting_method_tag = 1
