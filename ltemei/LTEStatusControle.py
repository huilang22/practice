...
#                print "[in thread:", threading.currentThread().getName(), "] now update view.";
                self.notifyDataUpdated(self._gui);
#                print "[in thread:", threading.currentThread().getName(), "]  Done(update view).";
            else:
...
class DataFetcherThread(threading.Thread):
    ...
    def run(self):
        ...
        # TODO: does any impacts when the application exiting?

    def doDataFetch(self):
#        print "[in thread:", threading.currentThread().getName(), "] start data-fetching...";
        if ( not self._fecher ):
...
#        print "[in thread:", threading.currentThread().getName(), "] Done. (the time consuming data-fetching), row=", len(self._data.getRealNodes());
...
#        print "[in thread:", threading.currentThread().getName(), "] set evt2.";
        ...
        gobject.idle_add(self._guiThread.copyDataFromWorkerThread, self._workerData);
