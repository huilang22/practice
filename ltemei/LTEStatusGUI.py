...
#gtk.rc_parse("/opt/nokia/oss/bin/LTEStatusGUI.rc")
...
#                print "to call onInitTableRightMenu: ", self.fp_onInitTableRightMenu;
    ...
    def create_table(self, window, container):
        ...
        self.liststore = self.liststore_list;
    ...
    def getRender(self,widgetDef,actionID, store, column):
        ...
        if(cmp(widgetDef,"checkbutton")==0):
            ...
            #widgetRender.set_property('cell-background-set', True);
        else:
            widgetRender = gtk.CellRendererText();
            #widgetRender.set_property('editable', True);
            # [xrg] why editable? change to false.
            widgetRender.set_property('editable', False);
            #widgetRender.set_property('cell-background','#C4C4C4');
            #widgetRender.set_property('cell-background-set', True);
...
#                print "showAlarmMonitor, gid=", gid;
                if ( "-3" == gid ):
                    ...
                    if ( not isinstance(activeHandler, LazyLoadingTreeHandlerBase) ):
                        raise Exception("has gid=-3 row, but not LazyLoadingTreeHandlerBase.");
...
class StatusReportActionsBase(GUIActions):
    ...
    def getCheckedNodes(self, action_name, gui):
        ...
        """
        checked_list = CheckboxColumnHelper.getCheckedList(gui);
        checked_nodes = [];
        for checked_dn in checked_list:            
            node = gui.data.getNode(checked_dn);
            if(not(node in checked_nodes)):
            ...
            for child in children:
                if(not(child in checked_nodes)):                    
                    checked_nodes.append(child);        
        return checked_nodes;

#################################################################################
# for lazy-loading framework
class ViewHandlerBase:
    ...
    # TODO: it's better to adding this in ColumnDef class
    #colIndexMap={};  # should include all columns, include invisible
    #visibleColumns = [];
...
#        print "[LTEActions.py], ", self.viewType, self, "-showView, start, time=", time.time();
        ...
        if( not gui.during_notifyDataUpdated ):
            gui.showStatus(gui.STATUS_MSG, "");
#        print "[LTEActions.py], ", self.viewType, self, "-showView, end, time=", time.time();
...
class ListHandler(ViewHandlerBase):
    ...
    def isFilteredNode(self, node, columnDef, gui):
        return columnDef.isFilteredNode(node,gui.filters);

    def addNodeTolist(self, gui, node):
        #listmodel = gui.w.tv.get_model();
        listmodel = gui.liststore;
        values = node.getRowValue().split(",");
        listmodel.append(values);

    def doesContainsFindingString(self, iter, keyword):
        return False;

class LazyLoadingTreeHandlerBase(ViewHandlerBase):
    def __init__(self):
        ViewHandlerBase.__init__(self);

        #  
        self.viewType = None;  # which tree type we are? can be any one viewType_xxx consts define in ViewHandlerBase

        # dummy GID for not expanded row
        self.dummyGID_tmp    = "0";
        self.dummyGID_OMS    = "-1";
        ...
        # hash to save the level1 iters
        self.L1IterMap = {};

        self.L1Hash = {};  # 

        # save the main gui object reference
        self._gui = None;
    ...
    # virtual method
    def getL1RealGID(self, dn):
        pass 
    
    def createNotExpandedMrsiteIter(self, mrsite, treemodel, parentIter=None):
        tmpNode = self._gui.data.createNode(self.dummyGID_MRSITE);
        # only need to fill DN and Mediator colomn
        tmpNode.set_property("DN","MRSITE-"+mrsite);
        tmpNode.set_property("MRSITE",mrsite);
        values = tmpNode.getRowValue().split(",");
        values[-1]=self._gui.ROW_COLORS[0];
        retIter = treemodel.append(parentIter, values);
        return retIter;

    def createNotExpandedOmsIter(self, meditor, treemodel, parentIter=None):
        tmpNode = self._gui.data.createNode(self.dummyGID_OMS);
        # only need to fill DN and Mediator colomn
        tmpNode.set_property("DN",meditor);
        tmpNode.set_property("Mediator",meditor);
        values = tmpNode.getRowValue().split(",");
        values[-1]=self._gui.ROW_COLORS[0];
        retIter = treemodel.append(parentIter, values);
        return retIter;
        
    def createNotExpandedDnIter(self, node, treemodel, parentIter=None):
        # failure1
        ## copy all values from the real dataNode, except the GID
        ## TODO: it's better to adding a copy-ctor in dataNode class.
        #tmpNode = node;  ## BUG ! this will cause the original dataNode's GID be changed (dummyGID_DN)
        #tmpNode.set_property('GID', self.dummyGID_DN);
        # failure2
        #tmpNode = copy.deepcopy(node);
        #deepcopy works for SD but not RFS, why? they are both LTEStatusDataNode instance...
        #anyway change to below:
#        print "ViewHandlerBase.visibleColumns=", ViewHandlerBase.visibleColumns;
        tmpNode = self._gui.data.createNode(self.dummyGID_DN);
        for col in ColumnDef.getInstance().getVisibleColumns():
            if (DataControl.getInstance().methodActions.has_key(col)):
                DataControl.getInstance().methodActions[col](tmpNode,node);
            else:
                tmpNode.set_property(col, node.get_property(col));
#                print col, '=', tmpNode.get_property(col);
        values = tmpNode.getRowValue().split(",");
        values[-1]=self._gui.ROW_COLORS[0];
        retIter = treemodel.append(parentIter, values);
        return retIter;

    # insert dummy child for the tree iter, so that a '+' is available for expanding
    def createDummyChild(self, treemodel, parentIter):
        tmpNode = self._gui.data.createNode(self.dummyGID_tmp);

        tmpNode.set_property("DN","Loading(subtree)...Please Wait.");
        values = tmpNode.getRowValue().split(",");
        values[-1]=self._gui.ROW_COLORS[0];
        return treemodel.append(parentIter, values);

    # udpate the value in each column with real data node.
    def updateRowValue(self, tree, iter, node):
        for column in ColumnDef.getInstance().getAllColumns():
            tree.set_value(iter, column.getColumnIndex(), node.get_property(column.ID));
    ...
    def _cbRestoreExpandedRows(self, model, path, iter, user_data):
        ...
        if path in self.expandedLines:
            treeview.expand_row(path, False); ## True:  recursively expand, 
                                              ## False: just expand immediate children

    def disconnect_expand_collapse(self, gui):
        if (hasattr(self, "expand_id") and self.expand_id):
            gui.w.tv.disconnect(self.expand_id);
            self.expand_id = None;
        if (hasattr(self, "collapse_id") and self.collapse_id):
            gui.w.tv.disconnect(self.collapse_id);
            self.collapse_id = None;
    def deactivateView(self, widget, gui):
#        print "[LTEActions.py, ", self.viewType, "-deactivateView.\n";
        self.disconnect_expand_collapse(gui);
        gui.w.tv.set_model(None);
        gui.liststore_tree.clear();
        self.L1IterMap.clear();
        self.L1Hash.clear();
        return;
    ...
    def activateView(self, widget, gui):
        ...
        # now we are in xxx[OMS/MRSITE/DN] Treeview.
        ViewHandlerBase.currentActivatedView = self.viewType;
        data = gui.data;
        columnDef = gui.data.getColumnDef();
        ...
        gui.w.tv.set_model(None);
        gui.w.tv.freeze_child_notify()
        gui.liststore = gui.liststore_tree;
        gui.liststore.clear();
        #gui.w.tv.connect('row-expanded', self.onTreeRowExpanded)
        #gui.w.tv.connect('row-collapsed', self.onTreeRowCollapsed)
        # if connect the "row-expanded", user need to click 2 times to got the row expanded!!!
        # ==> 
        # The "test-collapse-row" and "test-expand-row" signals are emitted before a row is collapsed or expanded. 
        # The return value from your callback can cancel or allow the operation
        # - TRUE to allow and FALSE to cancel.  
        #   def callback(treeview, iter, path, user_data)
        self.disconnect_expand_collapse(gui);  ## IMPORTANT!!! Disconnect any exsiting event handler
        self.expand_id   = gui.w.tv.connect('test-expand-row', self.onTreeRowExpanded)
        self.collapse_id = gui.w.tv.connect('test-collapse-row', self.onTreeRowCollapsed)
#        print "expand_id=", self.expand_id, "collapse_id=", self.collapse_id;

        self.L1IterMap.clear();
        self.L1Hash.clear();

        #gui.liststore.set_default_sort_func(lambda *args: -1);
        #gui.liststore.set_sort_column_id(-1, gtk.SORT_ASCENDING);
        ...
        for node in data.getRealNodes():
            ...
            if(self.isFilteredNode(node, columnDef, gui)):
                #print "Ignore filtered out node:", node.getDN();
                continue;
            self.fillLazyLoadingData(gui, node);
            #print "force update GUI...";
            while gtk.events_pending():
                gtk.main_iteration(False) 
            #print "force update GUI... end";
    ...
    ### virtual method
    def onBeforeFillingData(self):
        self.saveExpandedRowsIfNeeded(self._gui);

    ### virtual method
    def onAfterFillingData(self):
        self.restoreExpandedRowsIfNeeded(self._gui);
    
    ### virtual method, filling the lazy loading data. should implemented in drived class.
    ### normally insert the level-1 tree iters and save the data needed when expanding it.
    def fillLazyLoadingData(self, gui, node):
        pass

    # virtual method, should implemented in drived class.
    ...
    def onTreeRowExpanded(self, treeview, iter, path):
        ...
        #     "The return value from your callback can cancel or allow the operation
        #      - TRUE to allow and FALSE to cancel.  "
        # but when "return True;", it will always failed!
        # if "return TRUE;", got exception, and it's really expanded!!!
        # Traceback (most recent call last):
        #   File "/opt/nokia/oss/bin/LTEActions.py", line 219, in onTreeRowExpanded
        #     return TRUE;
    ...
    def onTreeRowCollapsed(self, treeview, iter, path):
        #print "onTreeRowCollapsed\n";
        return False;

    def insertDnSubTree(self, gui, parentIter, node, del_1st_dnpart=False):
        columnDef = gui.data.getColumnDef();

        if(not self.isFilteredNode(node, columnDef, gui)):
            myDN =  node.get_property("DN");
            dnPath = self.getDNPath(myDN);
#            print "getDNPath ret=", dnPath;
            treemodel = gui.liststore_tree;
            if (del_1st_dnpart):
                del dnPath[0];
                #print "dnPath=", dnPath;
            for dn in dnPath:
                # create the row (if parent not existing, create it.)
                iter = self.getIterByDN(gui, parentIter, dn);
                if ( dn == myDN ):
                    self.updateRowValue(treemodel, iter, node);
                parentIter = iter;
        #else:
        #    print "isFilteredNode", node.getDN();
...
#            print "NOTE!!! Ignore invalid DN:", myDN;
    ...
    def getIterByDN(self, gui, parentIter, dn):
        #print "getIterByDN,", dn;
        # TODO: adding quick access functionality here ...
        treemodel = gui.liststore_tree;
        iter = treemodel.iter_children(parentIter);
        while(iter != None):
            tmpDN = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('DN'));
            if ( tmpDN == dn ):
                break;
            iter = treemodel.iter_next(iter);
        
        if ( iter != None ):
            #print "existing row for dn", dn, iter;
            return iter;
        #else:
        # create a row with dummy data
        values = [];
        tmpNode = gui.data.createNode(self.dummyGID_tmp);
        tmpNode.set_property("DN",dn);
        values = tmpNode.getRowValue().split(",");
        values[-1]=gui.ROW_COLORS[0];
        iter = treemodel.append(parentIter, values);
        #print "create dummy row for dn", dn;
        return iter;
    ...
    def _getExportValuesInSubTree(self, iter):
        ...
        #values = [];
        # 1) export itself
        # 2) export children ( include not-loaded row in lazy-loading handler)
        #return values;
        pass
    ...
    def _getExportValueForDataNode(self, node):
        ...
        for col in ColumnDef.getInstance().getVisibleColumns():
            ...
            value += node.get_property(col);
        return value;
#end class LazyLoadingTreeHandlerBase

class DnTreeHandler(LazyLoadingTreeHandlerBase):
    def __init__(self):
        LazyLoadingTreeHandlerBase.__init__(self);

        self.viewType = ViewHandlerBase.viewType_DN;

        # 1 level hash table + 1 level(array) to save needed info to speed up the treeview showing
        # level-1:  key: level-1_DN value: level-2 array
        # level-2:  array, element is the real DataNode element
        #self.l2array = [];

        # hash to save the level1 dataNodes
        self.L1NodeMap = {};   # key: dn, value real dataNode

        # default L1 DN for RFS report and StatusDisplay report(with MRBTS not shown by default filter value)
        self.setL1DnPattern('^(.*LNBTS-\d+|.*WBTS-\d+)/.*$');
        # TODO: for 2G
        #self.setL1DnPattern('^(.*LNBTS-\d+|.*WBTS-\d+|.*BCF-\d+)/.*$');

    # virtual method
    def getL1RealGID(self, dn):
        if ( self.L1NodeMap.has_key(dn) ):
            node = self.L1NodeMap[dn];
#            print "[DnTreeHandler] getL1RealGID=", node.get_property('GID');
            return node.get_property('GID');
        return None;

    def setL1DnPattern(self, ptn):
        self.L1_DN_Pattern = ptn;
#        print "L1_DN_Pattern=", self.L1_DN_Pattern;

    def onBeforeFillingData(self):
        LazyLoadingTreeHandlerBase.onBeforeFillingData(self);
        self.L1NodeMap.clear();
...
#                print 'is_l1_dn', l1_dn, node;
                ...
                # lazy loading
                # create a dummy child since still not have one 
...
#                print "l2_array for l1_dn(", l1_dn, ")=", l2_array;
    ...
    #"row-expanded"      def callback(treeview, iter, path, user_param1, ...) 
    def onTreeRowExpanded(self, treeview, iter, path):
#        print self.viewType, "onTreeRowExpanded, start, time=", time.time()
        ...
        if ( self.dummyGID_DN == gid ):
            ...
            return self._expandL1Row(treeview, iter);
            
        #treeview.thaw_child_notify();
        #print "[LTEActions.py], onTreeRowExpanded, end, time=", time.time();

        return False;

    def _expandL1Row(self, treeview, iter):
        treemodel = treeview.get_model();

        # 0) remove the dummy children
        treestore = treemodel;
        dummy_child = treemodel.iter_children(iter);
        treestore.remove(dummy_child);
        # 1) set the GID to real gid, means it's already filled 1st-level DN row
        dn = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('DN'));
        if ( not self.L1NodeMap.has_key(dn) ):
            ## TODO: error handling!!!
            raise Exception("Error in code, L1NodeMap not has key", dn);
        node = self.L1NodeMap[dn];
        treemodel.set_value(iter, ColumnDef.getInstance().colIndexMap('GID'), node.get_property('GID')); 
        # 2) do really filling...
        if ( self.L1Hash.has_key(dn) ):
            l2_array = self.L1Hash[dn];
#            print "_expandL1Row,l2_array=", l2_array;
            if ( isinstance(l2_array, list) ):
                self._fillL2Array(iter, l2_array);
        else:
##            print "no really children";
            return True;  # important!!! otherwise, here will cause gtk lib assert failed.
        return False;

    def _fillL2Array(self, iter, l2_array):
        for node in l2_array:
            # insert the DN subtree
            self.insertDnSubTree(self._gui, iter, node, True);

    # return_value: an array, elem0: whether the @dn is Level-1 dn
    #                         elem1: the Level-1 dn part
    def getL1Dn(self, dn):
...
## aksfLNBTS-111
    ...
    #virtual method
    # Note: in currently implementation,
    #   for un-expanded-rows:
    #       NOT sorted as the view
    #   for expanded rows:
    #       sorted as view's sort order
    def _getExportValuesInSubTree(self, iter):
        treemodel = self._gui.liststore_tree;
        values = [];
        gid = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('GID'));
...
#    show_NE['WCEL']  = True;
    ...
    # vh -- viewHandler
    def saveSelectedAndCheckedRows(gui, vh):
        # checked rows
        CheckboxColumnHelper.checkedRows = [];
        CheckboxColumnHelper.checkedRows = CheckboxColumnHelper.getCheckedList(gui);

        # selected rows
        CheckboxColumnHelper.selectedRows = [];
        sel = gui.w.tv.get_selection();
        (model, pathlist) = sel.get_selected_rows();
        for i in pathlist:
            itr = model.get_iter(i);
            dn = model.get_value(itr, ColumnDef.getInstance().colIndexMap('DN'));
            CheckboxColumnHelper.selectedRows.append(dn);

        # expanded rows
        CheckboxColumnHelper.expandedRows = [];
        CheckboxColumnHelper.expandedRows = CheckboxColumnHelper.getExpanedRows(gui);
    ...
    def restoreSelectedAndCheckedRows(gui, vh):
        CheckboxColumnHelper._removeNotShownDN(CheckboxColumnHelper.checkedRows);
        CheckboxColumnHelper._removeNotShownDN(CheckboxColumnHelper.selectedRows);
        CheckboxColumnHelper._removeNotShownDN(CheckboxColumnHelper.expandedRows);

        treemodel = gui.liststore;
        iter = treemodel.get_iter_root();
        while(iter):
                                    
            if( len(CheckboxColumnHelper.checkedRows) == 0 and len(CheckboxColumnHelper.selectedRows) == 0 and len(CheckboxColumnHelper.expandedRows) == 0):
                break;
            if(len(CheckboxColumnHelper.checkedRows) != 0):
                CheckboxColumnHelper._restoreCheckedRows(gui, vh, treemodel, iter);
            if(len(CheckboxColumnHelper.selectedRows) != 0):
                CheckboxColumnHelper._restoreSelectedRows(gui, vh, treemodel, iter);
            if(len(CheckboxColumnHelper.expandedRows) != 0):
                CheckboxColumnHelper._restoreExpandedRows(gui, vh, treemodel, iter);
    ...
    def _restoreCheckedRows(gui, vh, treemodel, iter):
        dn = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('DN'));
#        print "checkedRows=", CheckboxColumnHelper.checkedRows, "dn=", dn
        if ( dn in CheckboxColumnHelper.checkedRows ):
            # ok, check it
            path = treemodel.get_path(iter);
            treemodel[path][gui.CHECKED_KEY] = True;
            # remove the checked dn, so we can abit speed up.
            CheckboxColumnHelper.checkedRows.remove(dn);
        if ( treemodel.iter_has_child(iter) and (len(CheckboxColumnHelper.checkedRows) > 0) ):
            for _dn in CheckboxColumnHelper.checkedRows:
                # vh must be DnTreeHandler(or drived)
                if (_dn.find(dn) != -1):
                    path = treemodel.get_path(iter);
                    gui.w.tv.expand_to_path(path);

                    child = treemodel.iter_children(iter);
                    while(child):
                        CheckboxColumnHelper._restoreCheckedRows(gui, vh, treemodel, child);
                        if ( len(CheckboxColumnHelper.checkedRows) == 0):
                            break;
                        child = treemodel.iter_next(child);
    _restoreCheckedRows = staticmethod(_restoreCheckedRows);

    def _restoreSelectedRows(gui, vh, treemodel, iter):
        dn = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('DN'));
#        print "selectedRows=", CheckboxColumnHelper.selectedRows, "dn=", dn
        if ( dn in CheckboxColumnHelper.selectedRows ):
            # ok, select it
            tree_selection = gui.w.tv.get_selection();
            tree_selection.select_iter(iter);
            CheckboxColumnHelper.selectedRows.remove(dn);

        if ( treemodel.iter_has_child(iter)  and (len(CheckboxColumnHelper.selectedRows) > 0) ):
            for _dn in CheckboxColumnHelper.selectedRows:
                # vh must be DnTreeHandler(or drived)
                if (_dn.find(dn) != -1):
                    path = treemodel.get_path(iter);
                    gui.w.tv.expand_to_path(path);
                    ...
                    child = treemodel.iter_children(iter);
                    while(child):
                        CheckboxColumnHelper._restoreSelectedRows(gui, vh, treemodel, child);
                        if ( len(CheckboxColumnHelper.selectedRows) == 0):
                            break;
                        child = treemodel.iter_next(child);
    _restoreSelectedRows = staticmethod(_restoreSelectedRows);

    def _restoreExpandedRows(gui, vh, treemodel, iter):
        dn = treemodel.get_value(iter, ColumnDef.getInstance().colIndexMap('DN'));
##        print "expandedRows=", CheckboxColumnHelper.expandedRows, "dn=", dn
        if ( (dn in CheckboxColumnHelper.expandedRows) ):
            path = treemodel.get_path(iter);
            if ( not gui.w.tv.row_expanded(path) ):
                # ok, expand it
                path = treemodel.get_path(iter);
                gui.w.tv.expand_row(path, False);
                CheckboxColumnHelper.expandedRows.remove(dn);
            else:
                CheckboxColumnHelper.expandedRows.remove(dn);

        if ( treemodel.iter_has_child(iter)  and (len(CheckboxColumnHelper.expandedRows) > 0) ):
            for _dn in CheckboxColumnHelper.expandedRows:
                # vh must be DnTreeHandler(or drived)
                if (_dn.find(dn) != -1):
                    path = treemodel.get_path(iter);
                    gui.w.tv.expand_to_path(path);
                    ...
                    child = treemodel.iter_children(iter);
                    while(child):
                        CheckboxColumnHelper._restoreExpandedRows(gui, vh, treemodel, child);
                        if ( len(CheckboxColumnHelper.expandedRows) == 0):
                            break;
                        child = treemodel.iter_next(child);
    _restoreExpandedRows = staticmethod(_restoreExpandedRows);
...
class ListHandler_LTEStatusDisplay(ListHandler):
    def __init__(self):
        ListHandler.__init__(self);
        self.need_restoreRows = False;

    def deactivateView(self, widget, gui):
        ToolbarStateReset.resetAllLockUnlockEtc(gui);
        ListHandler.deactivateView(self, widget, gui);

    def onBeforeFillingData(self):
        ListHandler.onBeforeFillingData(self);
        self.need_restoreRows = False;
        if ( ViewHandlerBase.currentActivatedView == self.viewType ):
            self.need_restoreRows = True;
            CheckboxColumnHelper.saveSelectedAndCheckedRows(self._gui, self);

    def onAfterFillingData(self):
        ListHandler.onAfterFillingData(self);
        if ( self.need_restoreRows ):
            CheckboxColumnHelper.restoreSelectedAndCheckedRows(self._gui, self);
        self._gui.action_invoker.doActions(None, GUIActions.UPDATE_ACTIONSTATE);
    
# end class ListHandler_LTEStatusDisplay(ListHandler):

class DnTreeHandler_LTEStatusDisplay(DnTreeHandler):
    # by default, the Level-1 dn is LNBTS
    def __init__(self):
        DnTreeHandler.__init__(self);

        self.setL1DnPattern('^(.*LNBTS-\d+)/.*$');

    def getDNPath(self, myDN):
        dnPath = DnTreeHandler.getDNPath(self, myDN);

##        print "show mrbts=", CheckboxColumnHelper.showNEType('MRBTS'), ", show_lnbts=", CheckboxColumnHelper.showNEType('LNBTS');
        if ( CheckboxColumnHelper.showNEType('MRBTS') and (not CheckboxColumnHelper.showNEType('LNBTS')) ):
            # in this case, do not create the LNBTS level tree node.
##            print "show mrbts and not show lnbts!!!";
            p = re.compile(".*LNBTS-\d+$");
            for dn in dnPath:
                if (p.match(dn)):
                    dnPath.remove(dn);
                    break;
        # TODO: if 3G StatusDisplay need to adding RNC,
        #       need to adding a simular code like above if clouse

        return dnPath;
    
    def deactivateView(self, widget, gui):
        ToolbarStateReset.resetAllLockUnlockEtc(gui);
        DnTreeHandler.deactivateView(self, widget, gui);

    def onBeforeFillingData(self):
        # at here we not call base!!!
        self.L1NodeMap.clear();
    ...
    def onAfterFillingData(self):
        DnTreeHandler.onAfterFillingData(self);
        if ( self.need_restoreRows ):
            CheckboxColumnHelper.restoreSelectedAndCheckedRows(self._gui, self);
        self._gui.action_invoker.doActions(None, GUIActions.UPDATE_ACTIONSTATE);
    ...
    def _fillL2Array(self, iter, l2_array):
        if ( CheckboxColumnHelper.showNEType('MRBTS') ):
            for node in l2_array:
                # insert the DN subtree
#                print "_fillL2Array, node=", node.get_property('DN'), node.get_property('GID');
                self.insertDnSubTree(self._gui, iter, node, False);
        else:
            DnTreeHandler._fillL2Array(self, iter, l2_array);
...
# end class DnTreeHandler_LTEStatusDisplay(DnTreeHandler):


# a (lazy) Finding Implement
# the handler Object should be a ViewHandlerBase object.
# i.e., implement method:
#     doesContainsFindingString
#     
class LazyFindingImpl:
    ...
    def get_model(self, gui):
        treemodel = None;
        if ( isinstance(self._handlerObj, LazyLoadingTreeHandlerBase) ):
            treeview = gui.w.tv;
            treemodel = treeview.get_model();
    ...
    def _findTheNextIter(self, currIter, ignore_crrrIter = True, ignore_subtree = False):
        ...
        # ==>
        retIter = None;
        bIsLazyLoadingView = isinstance(self._handlerObj, LazyLoadingTreeHandlerBase);
...
class DataExportBase:
    ...
    def saveBtnPressed(self, btn, chooser):
        '''
            Auto append '.csv' to the exported filename, 
        ...
        if ( filename[-4:].lower() != '.csv' ):
            ...
            #print ".csv auto apppended", filename;
    ...
    def getAllRowData(self, sep):
        self._sep = sep;
# export from data object directly!
#        values = [];
#        for node in gui.data.getRealNodes():
#            value = "";
...
class LazyDataExport(DataExportBase):
    def __init__(self, handlerObj):
        DataExportBase.__init__(self, handlerObj);
        if ( not isinstance(self._handlerObj, LazyLoadingTreeHandlerBase) ):
            exit("Error:LazyDataExportImpl with non-lazy-loading object.");
...
##    fakeData = ("1000002,PLMN-PLMN/MRBTS-666/LNBTS-666,, ,,,,,LN2.0;"
##                "1000003,PLMN-PLMN/MRBTS-701/LNBTS-701,,integrated to RAN,,,major:(1),10.9.162.15,LN2.0;"
##                "1000004,PLMN-PLMN/MRBTS-88888/LNBTS-701/LNCEL-11,MyTest,disable,unlocked,,critical:(1) major:(1),,LN1.0;"
##                "1000005,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-11,,disable,unlocked,,critical:(1) major:(1),,LNxxx;"
##                "1000006,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-12,,disable,unlocked,,critical:(1),,LN1.0;"
##                "1000007,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-13,lintu,disable,unlocked,,critical:(1) major:(1),,LN2.0;"
##                "1000008,PLMN-PLMN/MRBTS-701/LNBTS-706,, ,,,,,LN1.0;"
##                "1000009,PLMN-PLMN/MRBTS-701/LNBTS-706/LNCEL-11,, ,locked,,,,LN2.0;"
##                "1000010,PLMN-PLMN/MRBTS-702/LNBTS-702,Tulkku LNBTS,integrated to RAN,,,major:(22),10.9.162.17,LN2.0;"
##                "1000011,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-21,cell21,enable,,,major:(4),,LN2.0;"
##                "1000012,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-22,cel 22, ,locked,,,,LN2.0;"
##                "1000013,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-23,cel 23, ,locked,,,,LN2.0;"
##                "1000014,PLMN-PLMN/MRBTS-703/LNBTS-703,Tiainen LNBTS,onAir,,,major:(5) minor:(1),10.9.162.19,LN2.0;"
##                "1000015,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-0,cel 0,enable,unlocked,,,,LN2.0;"
##                "1000016,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-31,, ,locked,,,,LN2.0;"
##                "1000017,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-32,cel 32,enable,unlocked,,,,LN2.0;"
##                "1000018,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-33,cel 33,enable,unlocked,,,,LN2.0;"
##                "1000019,PLMN-PLMN/MRBTS-704/LNBTS-704,Varpunen,onAir,,,major:(5) minor:(2),10.9.162.20,LN2.0;"
##                "1000020,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-41,,enable,unlocked,,major:(2),,LN2.0;"
##                "1000021,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-42,,enable,unlocked,unknown,major:(1),,LN2.0;"
##                "1000022,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-43,,enable,unlocked,unknown,,,LN2.0;"
##                "1000023,PLMN-PLMN/MRBTS-706/LNBTS-706,,configured,,,critical:(1),10.9.162.22,LN2.0;"
##                "1000024,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-61,,disable,unlocked,unknown,,,LN2.0;"
##                "1000025,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-62,,disable,unlocked,unknown,,,LN2.0;"
##                "1000026,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-63,,disable,unlocked,,,,LN2.0;"
##                "1000027,PLMN-PLMN/MRBTS-708/LNBTS-708,Tiira,onAir,,,critical:(1),10.9.162.24,LN2.0;"
##                "1000028,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-81,,enable,unlocked,,,,LN2.0;"
##                "1000029,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-82,,enable,unlocked,,,,LN2.0;"
##                "1000030,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-83,,enable,unlocked,,,,LN2.0;"
##               );
    fakeData = ("99993,PLMN-PLMN/MRBTS-666/LNBTS-666,,Disabled(reset),,Major:(1),FALSE,,LN2.0;"
                "99994,PLMN-PLMN/MRBTS-701/LNBTS-701,,Disabled(commisioned),,Critical:(1) major:(1),FALSE,10.9.162.15,LN2.0;"
                "99995,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-11,,Disabled,Lock,Critical:(1),TRUE,,LN3.0;"
                "99996,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-12,,Enabled,Unlock,Critical:(1) major:(1),FALSE,,LN2.0;"
                "99997,PLMN-PLMN/MRBTS-701/LNBTS-701/LNCEL-13,lintu,Disabled,Lock,,FALSE,,LN1.0;"
                "99998,PLMN-PLMN/MRBTS-701/LNBTS-706,,Enabled(on air),,,FALSE,,LN1.0;"
                "99999,PLMN-PLMN/MRBTS-701/LNBTS-706/LNCEL-11,,Enabled,Unlock,,TRUE,,LN1.0;"
                "100000,PLMN-PLMN/MRBTS-88888/LNBTS-701,,Enabled(on air),,Critical:(1) major:(1),FALSE,,LN2.0;"
                "100001,PLMN-PLMN/MRBTS-88888/LNBTS-701/LNCEL-11,MyTest,,,Major:(22),FALSE,,LN2.0;"
                "100002,PLMN-PLMN/MRBTS-702/LNBTS-702,Tulkku LNBTS,Enabled(on air),,Major:(4),FALSE,10.9.162.17,LN2.0;"
                "100003,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-21,cell21,,,,TRUE,,LN2.0;"
                "100004,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-22,cel 22,,,,TRUE,,LN2.0;"
                "100005,PLMN-PLMN/MRBTS-702/LNBTS-702/LNCEL-23,cel 23,,,Major:(5) minor:(1),FALSE,,LN2.0;"
                "100006,PLMN-PLMN/MRBTS-703/LNBTS-703,Tiainen LNBTS,Enabled(on air),,,FALSE,10.9.162.19,LN2.0;"
                "100007,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-0,cel 0,Disabled,Lock,,TRUE,,LN2.0;"
                "100008,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-31,,Disabled,Lock,,FALSE,,LN2.0;"
                "100009,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-32,cel 32,Disabled,Lock,,FALSE,,LN2.0;"
                "100010,PLMN-PLMN/MRBTS-703/LNBTS-703/LNCEL-33,cel 33,,,Major:(5) minor:(2),TRUE,,LN2.0;"
                "100011,PLMN-PLMN/MRBTS-704/LNBTS-704,Varpunen,Enabled,Unlock,Major:(2),FALSE,10.9.162.20,LN2.0;"
                "100012,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-41,,Enabled,Unlock,Major:(1),TRUE,,LN2.0;"
                "100013,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-42,,Enabled,Unlock,,FALSE,,LN2.0;"
                "100014,PLMN-PLMN/MRBTS-704/LNBTS-704/LNCEL-43,,,,Critical:(1),FALSE,,LN2.0;"
                "100015,PLMN-PLMN/MRBTS-706/LNBTS-706,,Enabled(on air),,,FALSE,10.9.162.22,LN2.0;"
                "100016,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-61,,Disabled,Lock,,FALSE,,LN2.0;"
                "100017,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-62,,Disabled,Lock,,FALSE,,LN2.0;"
                "100018,PLMN-PLMN/MRBTS-706/LNBTS-706/LNCEL-63,,,,Critical:(1),FALSE,,LN2.0;"
                "100019,PLMN-PLMN/MRBTS-708/LNBTS-708,Tiira,Enabled(on air),,,FALSE,10.9.162.24,LN2.0;"
                "100020,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-81,,,,,,,;"
                "100021,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-82,,,,,,,;"
                "100022,PLMN-PLMN/MRBTS-708/LNBTS-708/LNCEL-83,,,,,,,;"
               );
    rows = fakeData.split(";");
    data = LTEStatusData();
#    data.setColumnDef("GID:visible=false,DN:expander=true;filter=NEType,Name,MRSITE");
    for i in range(len(rows)):
        if(cmp(rows[i], "") == 0):
            continue;
        node = data.createNode(rows[i]);
        data.addNode(node);
#    data.getColumnDef().setFilterToColumn({"DN":{"MRBTS":False, "LNBTS":True,"LNCEL":True}});
    tmsexample = LTEStatusGUI(data)
    tmsexample.show()
    # TODO: after fetch data from worker thread, this testing code is no more suitable...(not working now)
